...
function tempConverterToC(temperature) {
   let celsius = ((temperature - 32) / 1.8);
   return celsius;
}

function tempConverterToF(temperature) {
   let fahrenheit = ((temperature * 1.8) + 32);
   return fahrenheit;
}

document.getElementById("fahrenheit_to_celsius").addEventListener("click", tempConverterToC);
document.getElementById("celsius_to_fahrenheit").addEventListener("click", tempConverterToF);



// Use innerHTML on result id to show temp in the field kinda like this document.getElementById("myBtn").addEventListener("click", function(){
