...
//Old solution of adding buttons to questions using explicit iteration
// $("#surveyList li").each(function() {
//     var $removeButton = $('<button>').html("Remove question");
//     $(this).append($removeButton);
// });

//New solution using implicit iteration
$("#surveyList li").append($('<button>').html("Remove question"));

//Event listener on AddToSurvey button - creates new question in the survey list
...
//Event listeners for remove question buttons - using event delegation on all survey entries - removes entire entry that is being clicked
$("#surveyList").on('click', 'li', function(event) {
	event.preventDefault();
	this.remove();
});
