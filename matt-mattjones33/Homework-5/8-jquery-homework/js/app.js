...
$("#addQuestion").on("click", function(event) {
    ...
    let $removeButton = $('<button>').html("Remove question")
    var $newSurveyQuestion = $("#question").val();
    var $newLabel = $("<label>").html($newSurveyQuestion);
    var $newListItem = $("<li>").addClass("form-group padout").html($newLabel).append("<input type='text' class='form-control'>")
...
$("ol").on("click", "button", function(event) {
    event.preventDefault();
    var $deletedQuestion = $(this).parent("li");
    $deletedQuestion.hide(300);
});