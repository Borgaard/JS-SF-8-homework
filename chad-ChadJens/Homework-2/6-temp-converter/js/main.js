...
function convertFToC() {
   let degF = document.querySelector('#temperature').value;
   if (degF === "") {
      degF = 0;
   }
   let convC = (degF - 32) / 1.8;
   document.getElementById('result').innerHTML = degF + ' degrees F equals ' + convC.toFixed(1) + ' degrees C.';
   document.getElementById('temperature').value = null;
}

function convertCToF() {
   let degC = document.querySelector('#temperature').value;
   if (degC === "") {
      degC = 0;
   }
   let convF = (degC * 1.8) + 32;
   document.getElementById('result').innerHTML = degC + ' degrees C equals ' + convF.toFixed(1) + ' degrees F.';
   document.getElementById('temperature').value = null;
}

let fToCButton = document.getElementById('fahrenheit_to_celsius');
fToCButton.addEventListener('click', convertFToC);

let cToFButton = document.getElementById('celsius_to_fahrenheit');
cToFButton.addEventListener('click', convertCToF);