...
$('#addQuestion').on('click', function(event) {
	...
	let $newLiBlob = $('<li>').addClass('form-group padout');
	let $newLabelBlob = $('<label>').text(newQuestion);;
	let $newInputBlob = $('<input>').attr('type', 'text').addClass('form-control');
	$newLiBlob.append($newLabelBlob);
	$newLiBlob.append($newInputBlob);
	$newLiBlob.append($('<button>').html("Remove question"));
	$('#surveyList').append($newLiBlob);
})
